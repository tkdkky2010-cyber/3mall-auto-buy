# 롯데홈쇼핑 폰앱 결제 RUNBOOK (교과서) — ⚠️**폐기·히스토리 전용**

> ## ⛔ 보관본 — 현재 실행에 사용 금지
> 2026-05-27 실측 기록을 남긴 과거 문서다. **현재 실행 정본은
> `READ_FIRST.md` 의 "계정별 구매 필수 규약" + `LOTTE_HOMESHOPPING_STEPMAP.md` +
> `phone_auto/lotte_homeshopping_buy.py`** 다.
> 이 문서의 명령·좌표·기본값을 현재 구매에 적용하지 않는다. 특히 아래 두 가지가 **지금과 다르다**:
> - **Phase 3 의 결제설정 순서가 틀렸다** — 적립금/L.POINT 단계가 아예 없다.
>   정본 순서 = `주소 → 할인쿠폰 → 플러스쿠폰 → 적립금·L.POINT → 카드 → 현금영수증 → 동의`
>   (쿠폰이 포인트를 리셋 / 카드가 현금영수증을 리셋).
> - **Phase 2~3 의 `flow_runner` `lotte_*` 핸들러 + `coords/apps/lotte_homeshopping.json` 은
>   현재 아무도 호출하지 않는다.**
>
> 남겨 두는 이유는 Phase 4 카드별 경로·함정표가 참고가 되기 때문이다. **그 외는 읽지 말 것.**

**2026-05-27 END-TO-END 라이브 결제 성공 루트.** (당시 기록)
