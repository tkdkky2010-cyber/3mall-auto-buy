...
- **6자리 셔플** — 숫자 위치가 매번 바뀜.
- → **`input_pin source=text_dump`** 사용 (결제 순간마다 dump를 **라이브로 새로 읽어** 그때의 숫자→bounds 매핑으로 탭).
  ⚠️ **`source=dump`(content-desc) 아니다** — 2026-06-01 탐색 때 `dump` 로 적었으나, 7-15 라이브에서
  토스 키패드 숫자는 **`text` 노드**로 확인돼 `text_dump` 가 정본이다(문서 상단 박스와 동일). 2026-09-09 정정.
