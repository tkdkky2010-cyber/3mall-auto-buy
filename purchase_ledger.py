...
#   쿠폰 1장 누락 = 13,500~43,000원(상품 정가 135,000~270,000 × 10~15%)이라 2만원이면 걸린다.
#   ★최저가 상품 1개의 10%(≈13,500원)만 이론상 여유에 묻히는데, **실사용에선 안 생긴다** —
#     사용자 확인 2026-09-09: "최저가상품은 하나씩사지않음 무조건 최소 2개씩 삼, 그리고 다른
#     상품이랑 합쳐서 사기 때문에 + 2만원해도될듯". 2개면 27,000원이라 감지된다.
...
#                                  줄에서 읽어 되돌린다.
#    ★청구할인·페이백을 되돌리는 이유 (사용자 확인 2026-09-09, 문장 그대로):
#      "청구할인이랑 페이백은 우선 결제하고 나중에 받는 거기 때문에 저렇게 설정하면 안돼
#       item_final = 정가 × q × 0.9 × (1-쿠폰) + 20000 원으로 max 잡으면 적당할 거 같은데"
#    ★이 역산은 그 공식과 **수학적으로 동일**하다 — 시트 최종구매가가
#      `정가×q×0.9×(1-쿠폰)×(1-청구할인)×(1-페이백)` 이므로 뒤 두 항만 되돌리면 그대로 남는다.
#      2026-08-28 조합12 실측 대조: 직접계산 546,660 = 역산 546,660 (완전일치).
#      **직접 재계산하지 않고 역산하는 이유** = rate-check/lotte.py 의 compute() 를 두 벌로
#      복사하지 않으려는 것. 사본은 언젠가 어긋난다(READ_FIRST 「사본이 조용히 낡는다」).
