...
SAMPLE_TABLE: list[tuple[str, int, str | None]] = [
    # canonical (가이드 단가표와 동일 순서/표기)
    ("윤조에센스 8ml", 2_200, "s01"),
    ("윤조에센스 15ml", 4_300, "s02"),
    ("윤조마스크", 1_800, "s03"),
    ("자음수/유액 15ml", 3_300, "s04"),
    ("탄력크림 5ml", 1_100, "s05"),
    ("탄력크림 15ml", 3_400, "s06"),
    ("자음생수/유액 25ml", 4_800, "s07"),
    ("자음생세럼 15ml (캡슐세럼)", 11_500, "s08"),
    ("자음생세럼 5ml (캡슐세럼)", 2_000, "s09"),
    ("자음생세럼 8ml (캡슐세럼)", 3_800, "s10"),
    ("자음생크림 리치 5ml", 3_000, "s11"),
    ("자음생크림 리치 10ml", 8_000, "s12"),
    ("자음생크림 5ml", 2_100, "s13"),
    ("자음생아이크림 3ml", 2_000, "s14"),
    ("자음생 아이크림 4ml", 2_000, "s15"),
    ("자음생 왕 아이크림 5ml", 4_900, "s16"),
    ("자음생마스크", 3_300, "s17"),
    ("자음생브라이트닝세럼 8ml", 4_000, "s18"),
    ("자음생브라이트닝세럼 8ml (2026/02)", 3_800, "s19"),
    ...
    ("상백톤업선 10ml", 1_700, "s28"),
    ("설화수 선 플루이드 3ml", 400, "s29"),
    ("진생솝 25g", 1_500, "s30"),
    ("윤조 아이세럼 4ml", 2_900, "s31"),
    ("윤빛 마사저 1ea", 2_000, "s32"),
    ("자음생클렌징폼 50g", 5_000, "s33"),
    ("미니팩 3종", 7_300, "s34"),
    ("자정기미코렉터 3ml", 2_000, "s35"),
    ("자정수 25ml + 자정유액 25ml", 5_500, "s36"),
    ("자정앰플세럼 7ml", 4_500, "s37"),
    # SET 가격 규칙 (섹션 4) — 페이지에서 종종 SET 표기로 등장
    ("자음수유액SET", 3_300, "s04"),
    ("자음생수유액SET", 4_800, "s07"),
    # 옛 표기 alias (페이지가 새 이름으로 안 바뀐 경우 대비)
    ("윤조에센스 6세대 8ml", 2_200, "s01"),
    ("윤조에센스 6세대 15ml", 4_300, "s02"),
    ("자음수15ml자음유액15ml", 3_300, "s04"),
    ("자음생수25ml자음생유액25ml", 4_800, "s07"),
    ...
    ("상백선플루이드 3ml", 400, "s29"),
    ("순행클렌징폼 50ml", 2_400, "s22"),
    ("순행클렌징오일 50ml", 2_400, "s23"),
    ("자정수25ml자정유액25ml", 5_500, "s36"),
    ("탄력크림EX 15ml", 3_400, "s06"),
...
# ============================================================
GALLERIA_NAVER_MULT = 0.978  # 네이버 2.2% (1 - 0.022)
...
def lookup_sample(page_text: str) -> tuple[str, int, str | None] | None:
    ...
    candidates = []
    page_nosp = page_text.replace(" ", "")
    for name, price, code in SAMPLE_TABLE:
        n = _normalize(name)
        if n in norm or norm in n:
            # 타이브레이커: 정식이름이 page에 그대로 포함되면 우선
            # (괄호 안 batch 식별자가 normalize에서 제거돼 동일 길이가 되는 경우 대비)
            raw_match = 1 if name.replace(" ", "") in page_nosp else 0
            candidates.append((len(n), raw_match, name, price, code))
    if candidates:
        # 가장 긴 매치 우선 (구체적인 게 정확) → raw_match → 이름 lex
        candidates.sort(reverse=True)
        _, _, name, price, code = candidates[0]
