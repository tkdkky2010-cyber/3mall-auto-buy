...
- **모든 결과는 조합번호 1~11 순서로 시트 입력.** 공급률 오름차순·내림차순 정렬 절대 X.
...
## §13. 시트 5.15 (또는 "{M.DD}") 탭 layout 구조 — 사용자 5/15 확정

**구조만 기술 — 셀 값(공급률, 가격 등)은 매일 다르므로 박지 않는다.**

### 13-1. 행 영역 (vertical layout)
- **행 1~41**: 갤러리아 섹션 (제목 / 할인정보 / GWP 구성 / 추증가치 / 공급률 요약 / 우측 쿠폰 블록)
- **행 42~43**: 빈 (간격 좁힘 — 사용자가 조정)
- **행 44~60**: 현대Hmall 섹션 (제목 / 카드정보 / 11조합 공급률 요약)
- **행 61**: 빈
- **행 62~79**: 롯데 섹션 (제목 / 쿠폰/카드/적립 정보 / 11조합 공급률 요약 + 우측 쿠폰 블록)

→ 각 mall 스크립트의 `START` 변수:
- galleria.py: row 1부터 (write_grid start_row=1)
- _tmp/hmall_all.py: `START = 44`
- _tmp/lotte_all.py: `START = 62`

### 13-2. 3사 공급률 비교 차트 — J1:M12
오른쪽 상단에 배치 (Galleria 섹션과 같은 행 영역 1~12, 갤러리아 데이터 안 닿는 J~M열).

| 열 | 행 1 (header) | 행 2~12 (조합 1~11) |
|---|---|---|
| J | "조합" | 조합번호 (1~11) |
| K | "갤러리아몰" | 갤러리아 공급률 |
| L | "Hmall" | Hmall 공급률 |
...
- galleria.py: J1:K12 (조합번호 + 갤러리아 공급률)
- _tmp/hmall_all.py: L1:L12 (Hmall 공급률)
- _tmp/lotte_all.py: M1:M12 (롯데 공급률) + 조건부 서식 추가

### 13-3. 조건부 서식 — K2:M12 행별 최저값 강조
- **범위**: K2:M12 (3사 × 11조합)
...
- ⚠️ Galleria의 우측 쿠폰 블록(I~K, 행 29~36)과 비교 차트(J~M, 행 1~12)는 행 영역이 다르므로 충돌 없음. 단 갤러리아 K1 ("갤러리아몰" 비교 차트 헤더) vs Galleria 섹션 K행은 분리.
