...
- 옛 흐름 "장바구니에 b~h 한 번에 다 담고 옵션변경 +/− 로 조합별 수량 조정 / 체크박스 토글" 도 **폐기** (옵션변경 시 cart 리로드 + 인덱스/체크 리셋으로 오류 빈발).
- **현 흐름**: 조합마다 `clear_cart` → 본품 add (구매하기 → 옵션 → 장바구니 담기) → cart 진입 → "일반상품" 체크 → 구매하기 → **체크아웃 페이지 카드할인 캐러셀 자동 검출 → 당일 최대 할인 카드 1장 가격 추출** → 다음 조합 (clear → add 사이클 반복).
- 이유: 결제 페이지에 hidden 추가쿠폰이 적용될 수 있음 (장바구니에서는 보이지 않음). 매 조합 clear+add 가 비용은 들지만 인덱스/체크박스 정합성 깨질 위험 없음 (현재 검증된 안정 흐름, 2026-05-28).
...
### Hmall 체크아웃 페이지 카드할인 캐러셀
- 컨테이너: `.tarvxz0` 안 `<h2>` 텍스트 "카드할인" 으로 영역 식별.
- 그 캐러셀 첫 카드 = **당일 최대 할인율 카드** (편집 정렬). brand + percent + 미리보기 가격 자동 추출.
- 클릭 불필요 — 캐러셀 슬라이드 DOM 안에 `<img alt="cardCdNN">` (brand) + 할인율% 텍스트 + 미리보기 즉시할인후 가격이 같이 노출됨.
...
- **검증 패턴 — 임계값과 적립금 독립 matchAll 후 순서대로 페어링** (commit 98b26f2, 2026-05-17 multi-tier fix):
  - 임계값: `/([\d,]{3,})\s*원\s*이상/g`
  - 적립금: `/([\d,]{2,})\s*(?:원|P)\s*적립(?!금)/g`  +  보조 `/적립금\s*([\d,]{2,})\s*원/g`
  - 단일 한도: `/최대\s*([\d,]+)\s*(?:원|P)\s*적립/g`
- ⚠️ **사용 금지 (옛 버그 regex)**: `/([\d,]{3,})\s*원\s*이상[\s\S]{0,80}?([\d,]{2,})\s*(?:원|P)\s*적립/g` — lazy match가 두 tier 사이 80자 넘으면 두 번째 tier 누락 (SESSION_2026-05-15 버그 사례, 7500/15000 중 7500만 잡힘).
- `_check_lotte_reward.py` output 구조:
  - `verification.tier_rows = [{min, reward}, ...]` (임계값 오름차순) — **이게 SoT**. lotte.py 가 이걸 읽음.
  - `verification.max_pt` = 단일 한도 또는 tier_rows 최대 reward fallback. **본 스크립트 내부 print 전용**, lotte.py 무시.
- **정상 패턴**: 롯데 이벤트는 결제금액 기반 → 7상품 모두 같은 tier 리스트 (예: 50000↑/7500원, 150000↑/15000원) 나오는 게 정상. 동일 = 의심 아님.
- **의심 케이스 (이때만 점검)**: ① tier_rows 빈 리스트 ② tier 1개뿐인데 페이지엔 여러 구간 보임 ③ threshold 비현실적.
...
### 13-4. 카트플랜 출력 — O1:U{end}
cart_plan.py 가 작성. 자동 채널 선택 + N개 분배 결과 (Step 1 끝 자동 실행).
- O1: "카트 플랜" / P1: 채널 / Q1: f"N={n}" / R1: 날짜 (M.DD)
- O2:R2: 헤더 ["조합번호", "조합", "구매수", "공급률"]
- O3~ : 데이터 행 (조합번호 순 정렬, qty>0 만 — qty 는 1이 아니라 2~ 일 수도 있음)
- summary 행: ["{channel}", "총 회수", "총 결제", "평균 공급률"]
- (빈 행)
- "[b~h 제품별 총 수량 (본)]" 타이틀
- "b 윤3", "c 자음2", ..., "h 자생크림" 헤더 (O~U, 7컬럼)
- 수량 행 (b~h 각 코드별 총 본품 수 — qty × 조합 내 개수 누적)

**분배 로직 — 조합 중복 허용** (RULES §13-4-1):
- `make_cart_plan` round-robin: `idx = available[slot % len(available)]`
- N > 가용 조합 수 → 자동으로 같은 조합 2~3개씩 (예: N=14 + 가용 7개 → 각 2개씩, N=36 + 가용 7개 → 5~6개씩).
- 친구 카드 N=14/36 시나리오에서 이게 정상 동작이며 의도된 결과.
