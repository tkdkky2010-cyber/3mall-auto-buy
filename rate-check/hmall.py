...
def main(argv=None):
    ...
    # composition 로드 (추가증정가치 + GWP 6세트)
    from datetime import datetime
    today = datetime.now().strftime("%Y-%m-%d")
    comp_path = C.TMP_DIR / f"today_composition_{today}.json"
    if not comp_path.exists():
        sys.exit(f"❌ {comp_path} 없음 — 먼저 galleria.py 실행 필요")
    comp = json.loads(comp_path.read_text(encoding="utf-8"))
    add_gift_value = {c: comp["products"][c]["add_gift_value"] for c in comp["products"]}
    gwp_6set = comp["gwp"]["6set_value"]
    print(f"[INFO] composition: {comp_path.name} | GWP 6세트={gwp_6set:,}원")
